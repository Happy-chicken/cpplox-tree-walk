...
shared_ptr<Stmt> Parser::ifStatement() {
  ...
  while (match({ELIF})) {
    ...
    elif_branches.emplace_back(std::move(elif_condition),
                               std::move(elif_branch));
  }
...
shared_ptr<Stmt> Parser::controlStatement() {
  ...
  if (keyword.type == BREAK) {
    ...
    consume(SEMICOLON,
            "Syntax Error. Syntax Error. Expect ';' after 'continue'.");
...
shared_ptr<Expr<Object>> Parser::comparison() {
  auto expr = binary([this]() { return term(); },
...
shared_ptr<Expr<Object>> Parser::term() {
  auto expr = binary([this]() { return factor(); },
...
shared_ptr<Expr<Object>> Parser::factor() {
  auto expr = binary([this]() { return unary(); },
                     {TokenType::SLASH, TokenType::BACKSLASH, TokenType::STAR,
                      TokenType::MODULO});
  return expr;
}
...
runtime_error Parser::error(Token token, string message) {
  ...
  if (token.type == TOKEN_EOF) {
    ...
    return runtime_error(to_string(token.line) + " at '" + token.lexeme + "'" +
                         message);
  }
}
