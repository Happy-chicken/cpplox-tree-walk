...
Object PrintCallable::call(shared_ptr<Interpreter> interpreter,
...
Object InputCallable::call(shared_ptr<Interpreter> interpreter,
                           vector<Object> args) {
...
std::string stringify(const Object &item, std::stringstream &stream) {
  ...
  if (std::holds_alternative<double>(item.data)) {
    ...
    num_as_string.erase(num_as_string.find_last_not_of('0') + 1,
                        std::string::npos);
    num_as_string.erase(num_as_string.find_last_not_of('.') + 1,
                        std::string::npos);
