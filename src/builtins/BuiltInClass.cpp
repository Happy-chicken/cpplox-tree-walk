...
ListClass::ListClass() : LoxClass("list", nullptr, {}) {
  this->methods["len"] = std::make_shared<ListLenMethods>(
      ...
      std::make_shared<Function>(Token(), vector<Token>{},
                                 vector<shared_ptr<Stmt>>{}));
  this->methods["append"] = std::make_shared<ListAppendMethods>(
      std::make_shared<Environment>(),
      std::make_shared<Function>(Token(), vector<Token>{Token()},
                                 vector<shared_ptr<Stmt>>{}));
}
...
Object ListClass::call(shared_ptr<Interpreter> interpreter,
...
// ------------------------------------------------------------------------------------------
ListLenMethods::ListLenMethods(shared_ptr<Environment> closure_,
                               shared_ptr<Function> declaration_)
    : LoxFunction(declaration_, closure_, false) {}

Object ListLenMethods::call(shared_ptr<Interpreter> interpreter,
                            std::vector<Object> args) {
  size_t length =
...
// ------------------------------------------------------------------------------------------
ListAppendMethods::ListAppendMethods(shared_ptr<Environment> closure_,
                                     shared_ptr<Function> declaration_)
    : LoxFunction(declaration_, closure_, false) {}

Object ListAppendMethods::call(shared_ptr<Interpreter> interpreter,
                               std::vector<Object> args) {
