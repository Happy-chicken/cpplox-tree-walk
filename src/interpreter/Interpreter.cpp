...
Interpreter::Interpreter() //: global_environment{globals.get()}
{
  ...
  // native class
  globals->define("list",
                  Object::make_class_obj(std::make_shared<ListClass>()));
...
void Interpreter::executeBlock(vector<shared_ptr<Stmt>> statements,
...
void Interpreter::checkNumberOperands(Token operation, Object left,
                                      Object right) {
...
Object Interpreter::visitIncrementExpr(shared_ptr<Increment<Object>> expr) {
  ...
  if (old_value.data.index() == 1) {
    throw RuntimeError(expr->identifier,
                       "Runtime Error. Cannot increment a non integer type '" +
...
Object Interpreter::visitSubscriptExpr(shared_ptr<Subscript<Object>> expr) {
  ...
  if (iden_ptr.data.index() != 4) {
    throw RuntimeError(expr->identifier, "Runtime Error. Object " +
                                             expr->identifier.lexeme +
                                             "can not be subsripted");
  }
  ...
  try {
    ...
    throw RuntimeError(expr->identifier,
                       "RunTime Error. Index out of range. Index is " +
                           to_string(static_cast<int>(index_cast)) +
                           " but object size is " + to_string(object_size));
  }
}
...
Object Interpreter::visitCallExpr(shared_ptr<Call<Object>> expr) {
  ...
  if (std::holds_alternative<shared_ptr<LoxCallable>>(
          callee.data)) { // callee.type == Object::Object_fun
  ...
  if (arguments.size() != callable->arity()) {
    throw RuntimeError(expr->paren, "Runtime Error. Expected " +
                                        to_string(callable->arity()) +
                                        " arguments but got " +
...
Object Interpreter::visitSuperExpr(shared_ptr<Super<Object>> expr) {
  ...
  if (method == nullptr) {
    throw RuntimeError(expr->method, "Runtime Error. Undefined property '" +
                                         expr->method.lexeme + "'.");
  }
