...
template <class R> class Increment;

template <class R> class Decrement;

template <class R> class List;

template <class R> class Subscript;
...
public:
  typedef enum { POSTFIX, PREFIX } Type;
...
public:
  Logical(shared_ptr<Expr<R>> left_, Token operation_,
...
public:
  ...
  Subscript(Token identifier_, shared_ptr<Expr<R>> index_,
            shared_ptr<Expr<R>> value_)
...
public:
  Call(shared_ptr<Expr<R>> callee_, Token paren_,
